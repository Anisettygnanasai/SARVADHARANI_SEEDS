...
import { createCompany, deleteCompany, fetchAdminCompanies, inviteAdmin, updateCompany, fetchPendingUsers, fetchCompanyUsers, approveOrRejectUser, updateUserRole, deleteUserById } from "../services/authService";
...
export default function DashboardPage({ user }) {
  ...
  const [pending, setPending] = useState([]); const [employees, setEmployees] = useState([]); const [companies, setCompanies] = useState([]); const [msg, setMsg] = useState("");
  ...
  const loadAdminData = () => Promise.all([fetchPendingUsers(), fetchAdminCompanies(), fetchCompanyUsers()]).then(([p, c, u]) => { setPending(p.data || []); setCompanies(c.data || []); setEmployees(u.data || []); });
  ...
  const handleApprove = async (user_id) => { await approveOrRejectUser({ user_id, action: "approve" }); setMsg("User approved"); loadAdminData(); };
  const handleReject = async (user_id) => { await approveOrRejectUser({ user_id, action: "reject" }); setMsg("User rejected"); loadAdminData(); };
  const handleRoleChange = async (user_id, new_role) => { await updateUserRole({ user_id, new_role }); setMsg("Role updated"); loadAdminData(); };
  const handleDeleteUser = async (user_id) => { await deleteUserById(user_id); setMsg("User deleted"); loadAdminData(); };
  ...
  if (isMainAdmin) return (
    <div className="space-y-5">
      ...
      <div className="glass-card p-4"><h3 className="mb-2 text-lg font-semibold">Pending User Approvals</h3>{pending.length === 0 ? <p>No pending users.</p> : pending.map((u) => <div key={u.id} className="mb-2 flex items-center justify-between rounded-lg border border-white/10 p-2"><span>{u.full_name} ({u.email}) - {u.company_name || "N/A"}</span><div className="flex gap-2"><button className="btn-primary" onClick={() => handleApprove(u.id)}>Approve</button><button className="btn-danger" onClick={() => handleReject(u.id)}>Reject</button></div></div>)}</div>
      ...
      <div className="glass-card p-4"><h3 className="mb-2 text-lg font-semibold">Company Employees</h3><div className="space-y-2">{employees.map((u) => <div key={u.id} className="flex items-center justify-between rounded-lg border border-white/10 p-2"><span>{u.name} ({u.email}) - {u.company_name || "N/A"} - {u.approval_status || u.status}</span><div className="flex items-center gap-2"><select className="input-premium" value={u.role} onChange={(e) => handleRoleChange(u.id, e.target.value)} disabled={u.is_main_admin}><option value="accountant">accountant</option><option value="admin">admin</option></select><button className="btn-danger" onClick={() => handleDeleteUser(u.id)}>Delete</button></div></div>)}</div></div>
