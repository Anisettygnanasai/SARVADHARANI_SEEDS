...
export default function DashboardPage({ user }) {
  ...
  if (isMainAdmin) return (
    <div className="space-y-5">
      ...
      <div className="glass-card p-4"><h3 className="mb-2 text-lg font-semibold">Pending User Approvals</h3>{pending.length === 0 ? <p>No pending users.</p> : pending.map((u) => <div key={u.id} className="mb-2 flex items-center justify-between rounded-lg border border-white/10 p-2"><span>{u.full_name} ({u.email})</span><div className="flex gap-2"><button className="btn-primary" onClick={() => handleApprove(u.id)}>Approve</button><button className="btn-danger" onClick={() => handleReject(u.id)}>Reject</button></div></div>)}</div>
      ...
      <div className="glass-card p-4"><h3 className="mb-2 text-lg font-semibold">Company Employees</h3><div className="space-y-2">{employees.map((u) => <div key={u.id} className="flex items-center justify-between rounded-lg border border-white/10 p-2"><span>{u.name} ({u.email}) - {u.approval_status || u.status}</span><div className="flex items-center gap-2"><select className="input-premium" value={u.role} onChange={(e) => handleRoleChange(u.id, e.target.value)} disabled={u.is_main_admin}><option value="accountant">accountant</option><option value="admin">admin</option></select><button className="btn-danger" onClick={() => handleDeleteUser(u.id)}>Delete</button></div></div>)}</div></div>
