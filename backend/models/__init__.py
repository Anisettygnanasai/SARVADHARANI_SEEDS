...
class StockItem(db.Model, TimestampMixin):
    ...
    quantity_on_hand = db.Column(db.Numeric(18, 3), nullable=False, default=0)
...
class Transaction(db.Model, TimestampMixin):
    ...
    transaction_timestamp = db.Column(db.DateTime(timezone=True), default=lambda: datetime.now(timezone.utc), nullable=False)
    created_by_user_id = db.Column(db.BigInteger, db.ForeignKey("users.id"), nullable=False)
...
class TransactionItem(db.Model):
    ...
    transaction = db.relationship("Transaction", backref=db.backref("items", lazy=True, cascade="all, delete-orphan"))
...
class StockHistory(db.Model):
    ...
    quantity_before = db.Column(db.Numeric(18, 3), nullable=False)
